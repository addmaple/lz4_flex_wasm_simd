...
fn compress_raw(input: &[u8]) -> Option<Vec<u8>> {
    let mut out = Vec::new();
    if input.len() <= u16::MAX as usize {
        let mut table = lz_fear::raw::U16Table::default();
        lz_fear::raw::compress2(input, 0, &mut table, &mut out).ok()?;
    } else {
...
fn decompress_raw(input: &[u8], expected_len: usize) -> Option<Vec<u8>> {
    let mut out = Vec::with_capacity(expected_len);
    lz_fear::raw::decompress_raw(input, &[], &mut out, usize::MAX).ok()?;
    Some(out)
}
...
pub extern "C" fn wasm_block_roundtrip() -> i32 {
    ...
    let compressed = match compress_raw(&input) {
        Some(v) => v,
        None => return 0,
    };
    match decompress_raw(&compressed, input.len()) {
        Some(decoded) if decoded == input => 1,
        _ => 0,
    }
}
